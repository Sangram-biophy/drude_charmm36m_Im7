...
if ff=="CHARMM":
    selections = [
        u.select_atoms(f"protein and (resid {i} and not name C O H NH CA HA)")
        for i in range(np.unique(protein.resids)[0], np.unique(protein.resids)[-1]+1)
    ]
elif ff=="DRUDE":
    selections = [
        u.select_atoms(f"protein and (resid {i} and not name LPOA LPOB C DC O DO N DN HN DCA CA HA)",updating=True)
        for i in range(np.unique(protein.resids)[0], np.unique(protein.resids)[-1]+1)
    ]
...
dipole_moment_peptide = np.zeros((n_frames, protein.n_residues))  # Store x, y, z dipole components
...
if ff=="CHARMM":
    np.save("residuewise_side_chain_dipole_moment_time_evol.npy",dipole_moment_peptide,allow_pickle=True)
if ff=="DRUDE":
    np.save(f"residuewise_DRUDE_LP_include_side_chain_dipole_moment_time_evol_skip{skip}.npy",dipole_moment_peptide,allow_pickle=True)
